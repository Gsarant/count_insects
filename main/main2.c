...
#define QUALITY_OF_JPG_IMAGE 80
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Current  Config Version %lf",rec_config_file.version_config);
        ...
        if(now1<100){
                    ...
                    if(gsm_connected){  
                        ...
                        xEventGroupWaitBits(sntp_event_group, BIT0, false, true,portMAX_DELAY);
                        ...
                        #ifdef SLEEP
                            ESP_LOGD(TAG,"Sleep for %d Seconds ",5);
                            uint64_t sleeptime1 = UINT64_C(5 * uS_TO_S_FACTOR);
                            esp_sleep_enable_timer_wakeup(sleeptime1);
    ...
    void init_sntp(void *pvParameters){
        ...
        xEventGroupSetBits(sntp_event_group, BIT0);
        vTaskDelete(NULL);
    }
#endif//REALTIMECLOCK
...
void send_image(Rec_config_file_t *rec_config_file ,char *jpg_file ,uint8_t* _jpg_buf,size_t _jpg_buf_len){
    ...
    if (_jpg_buf_len>0) {
        ...
        BaseType_t mytask=xTaskCreate(&http_task_send_image, "http_task", 40000,  & rec_parameters_http_task, tskIDLE_PRIORITY, &xHandle);
